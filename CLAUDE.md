...
**UI-Struktur ("Studio"-Optik):** Vorschau-Video ganz oben (`.video-preview` mit `max-height: 42vh`, damit ein Hochformat-Clip nicht die ganze Seite füllt und Clip-Strip/Timeline darunter noch Platz haben), darunter der kompakte `#clipStrip` (ein Block pro Clip — Nummer, Name, Frame-Anzahl, Klick macht ihn zu `activeClipId`) und darunter `#clipTimelineWrap` mit der frame-genauen Mini-Timeline (In/Out/Cut-Point-Drag) des aktiven Clips. Rechts die Sidebar: Render-/Download-/Export-Buttons fest oben (`position: sticky`, wie im Fotomodus), darunter `#clipDetail` — ein unabhängig scrollbarer Stapel aus einklappbaren Karten (Zuschnitt/Farbe/Farbstich/Datamosh/Glitch-Filter, `buildCollapsibleSection()`, Zustand in `sectionOpen` geteilt über alle Clips hinweg) für den aktiven Clip. Trennung von Timeline (Hauptspalte) und übrigen Einstellungen (Sidebar) ist bewusst: die Vorschau bleibt beim Scrollen durch die Einstellungen immer im Blick.

**Rendern ist bewusst rein manuell** (kein Auto-Render): "Jetzt aktualisieren" ruft `resolveDirtyClips()` (preparet jeden Clip mit `needsReprepare` neu — z.B. nach einer Zuschnitt-Änderung) gefolgt von `doRender(false)`. Beide Action-Buttons (`cleanBtn`/`renderBtn`) werden während der Ausführung deaktiviert und zeigen Text-Feedback ("Aktualisiert…"), das schließt Doppel-Klicks/überlappende Requests aus. Ein früherer Debounce-Auto-Render-Ansatz wurde nach Nutzer-Feedback wieder entfernt — Änderungen wirken erst nach explizitem Klick, kein Live-Update während des Reglerziehens. Fehler beim Rendern zeigen jetzt einen `alert()` statt nur einen Konsolen-Log (war zuvor ein Grund, warum der Button "nicht zu funktionieren schien" — Fehler wurden früher, im Auto-Render-Kontext, bewusst still verschluckt).
...
4. Pro Clip einstellbar (Sidebar, einklappbare Karten): Zuschnitt, Farbe, Datamosh-Parameter, Glitch-Filter
5. "Jetzt aktualisieren" klicken, um zu rendern; "Ohne Moshing ansehen" zeigt die Referenz ohne Mosh
...
**Warum kein ffmpeg.wasm:** die Byte-Manipulation selbst ist reines JS (schnell, interaktiv, kein Encoder nötig). Nur Encode (prepare) und Decode (render) brauchen einen echten Codec — dafür reicht ein schlanker Server-Endpoint, ohne WASM-Overhead im Browser. Das ist auch der Grund, warum eine echte Timeline-Live-Vorschau (Scrubbing zeigt sofort die Mosh-Korruption) nicht möglich ist — der Melt-Effekt entsteht erst beim echten Decodieren fehlender I-Frames durch ffmpeg/libavcodec, kein Browser-`<video>`-Element bildet das nach. Rendern bleibt deshalb bewusst manuell (siehe oben) statt eine Live-Vorschau vorzutäuschen.

**Leinwand-System / Canvas-Fit:** analog zum Foto-Modus-Prinzip legt entweder der erste hochgeladene Clip automatisch eine Ziel-Auflösung fest (anhand seines eigenen Seitenverhältnisses), oder der "Leinwand"-Button öffnet ein Modal mit Presets (9:16/1:1/16:9) + Custom-W/H + Hintergrundfarbe. `canvasCfg = {w,h,bg}` (Client-State, Proxy-Auflösung ≤640px lange Kante) wird bei jedem `/prepare`-Aufruf mitgeschickt; der Server (`build_fit_filter()` in `server/app.py`) baut je nach `fitMode` eine von drei ffmpeg-Filterketten: **cover** (hochskalieren + Überschuss croppen, `panX/panY` verschieben den Ausschnitt), **contain** (runterskalieren + mit `bg` auffüllen), **stretch** (Seitenverhältnis ignorieren, bewusst nutzbare Option für zusätzliche Verzerrung). Jeder Clip hat sein eigenes `fit = {mode, panX, panY}`; eine Änderung setzt nur `clip.needsReprepare = true` (keine sofortige Aktion) — `resolveDirtyClips()` arbeitet das beim nächsten Klick auf "Jetzt aktualisieren" oder "Ohne Moshing ansehen" ab (ruft `/prepare` nur für die tatsächlich geänderten Clips erneut auf, nicht für alle).
...
**Farbstich pro Clip (seit v2.10.0):** färbt den Clip Richtung einer gewählten Farbe (z.B. Grün für CCTV-/Nachtsicht-Look), `clip.tint = {enabled, color, mix}`. Server (`hex_to_hue_sat()` + `build_segment_filter()`) leitet aus der Hex-Farbe per `colorsys.rgb_to_hls()` Hue (0–360°) und Sättigung (0–1) für ffmpeg's `colorize`-Filter ab (Lightness fix bei 0.5 — bewahrt die Original-Helligkeitsstruktur statt alles auf eine Fläche zu flachen). Da `colorize` das Bild komplett ersetzt, für eine stufenlose Stärke aber die *Originalfarben* noch durchscheinen müssen, reicht ein einfacher Comma-Chain-Filter nicht — stattdessen `split=2` → eine Kopie durch `colorize`, dann `blend=all_mode=normal:all_opacity=X` mit der unveränderten Kopie zusammenmischen. Einziger Filter in dieser Datei, der pro Segment einen eigenen Mini-Filtergraph statt eines linearen Comma-Chains braucht (`build_segment_filter()`s `base_chain`/`tail_chain`-Aufteilung existiert genau deswegen).
**Stolperfalle, die genau hier auftrat:** ffmpegs `blend`-Filter-Parameter `all_opacity` ist **invertiert** zur naheliegenden Annahme — `all_opacity=1` zeigt zu 100% die *untere* (Original-)Ebene, `all_opacity=0` zu 100% die *obere* (gefärbte) Ebene. Beim ersten Bauen führte die naive Annahme ("höherer Wert = mehr Effekt") dazu, dass der Regler bei hoher "Stärke" fast unsichtbar war und bei niedriger Stärke am stärksten wirkte — nur durch numerisches Pixel-Sampling (nicht durch visuelles Prüfen von Vorschaubildern) gefunden. Server rechnet deshalb `opacity = 1 - mix`, mit Kommentar im Code. Lehre: bei jedem neuen ffmpeg-Filter-Parameter, dessen Richtung nicht hundertprozentig aus der Doku hervorgeht, Pixelwerte an mehreren Reglerständen (0/50/100%) numerisch vergleichen, nicht nur ein Vorschaubild anschauen — visuelle Fehleinschätzungen bei kleinen/subtilen Verschiebungen sind leicht möglich.
...
**Export in hoher Qualität:** die editierte Mosh-Entscheidung (In/Out, Cut-Point, Wiederhol-/Rausch-Parameter) ist immer eine Liste von Frame-*Indizes* — die bleibt gültig, egal bei welcher Auflösung ein Clip encodiert wurde, solange FPS/GOP gleich bleiben (nur die Zuschnitt-Filterkette ändert sich). Der "Exportieren"-Button skaliert `canvasCfg` proportional auf die gewählte Export-Zielgröße hoch (720p/1080p, lange Kante) und re-preparet damit alle Clips (inkl. ihrer jeweiligen `fit`-Einstellung) neu, wendet dieselben Trim/Cut-Point-Werte an (defensiv auf neue Frame-Anzahl geclampt) und rendert mit `crf=16` statt `20`. Deutlich langsamer als ein normales "Jetzt aktualisieren" (mehrere Server-Roundtrips), aber nur beim finalen Export nötig.
...
| v2.9.0 | Video: Layout-Überarbeitung (Vorschau oben, Timeline darunter, Einstellungen als scrollbare einklappbare Karten in der Sidebar, Buttons bleiben fix), Auto-Render nach Nutzer-Feedback wieder entfernt zugunsten von zuverlässigem manuellem "Jetzt aktualisieren" | ✅ done |
| v2.10.0 | Video: Vorschau-Höhe gedeckelt (Hochformat verdrängt Timeline nicht mehr), Farbstich-Effekt pro Clip (CCTV-/Duotone-Look, ffmpeg `colorize`+`blend`) | ✅ done |
...
- Kein echtes Frame-genaues Scrubbing wie in einem Schnittprogramm und kein Live-Preview während des Reglerziehens — Rendern ist bewusst rein manuell ("Jetzt aktualisieren"), ein früherer Debounce-Auto-Render-Ansatz wurde nach Nutzer-Feedback wieder entfernt
