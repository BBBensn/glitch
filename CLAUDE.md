...
**UI-Struktur ("Studio"-Optik):** Vorschau-Video ganz oben, darunter der kompakte `#clipStrip` (ein Block pro Clip — Nummer, Name, Frame-Anzahl, Klick macht ihn zu `activeClipId`) und darunter `#clipTimelineWrap` mit der frame-genauen Mini-Timeline (In/Out/Cut-Point-Drag) des aktiven Clips. Rechts die Sidebar: Render-/Download-/Export-Buttons fest oben (`position: sticky`, wie im Fotomodus), darunter `#clipDetail` — ein unabhängig scrollbarer Stapel aus einklappbaren Karten (Zuschnitt/Farbe/Datamosh/Glitch-Filter, `buildCollapsibleSection()`, Zustand in `sectionOpen` geteilt über alle Clips hinweg) für den aktiven Clip. Trennung von Timeline (Hauptspalte) und übrigen Einstellungen (Sidebar) ist bewusst: die Vorschau bleibt beim Scrollen durch die Einstellungen immer im Blick.
